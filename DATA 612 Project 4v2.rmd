...
This recommender system uses the `recommenderlab` package, and we'll load the MovieLense dataset. We can see that using the `class()` function, that this MovieLense dataset is a `realRatingMatrix`. This is a continuation of 
...
## Splitting Training & Testing Sets with Multiple Methods

We will proceed by testing the performance using algorithms so far discussed this semester, in addition to different evaluation scheme methods.
...
### Split into Training & Test datasets using an 80/20 Ratio (Split)
...
### Split into Training & Test datasets using an 80/20 Ratio (Cross-Validation)
...
### Split into Training & Test datasets using an 80/20 Ratio (Bootstrap)
...
## Evaluating Algorithms Using Multiple Evaluation Schemas
...
```{r, fig.show="hold", out.width="50%", fig.height=5, echo=F}
